...
function BlockchainDouble(options) {
  ...
  // Jul: time adjustment is some kind of neat hack for debugging. Needs removal
...
const defaultOptions = {
  ...
  // Jul: this flag should be removed
...
BlockchainDouble.prototype.initialize = function(accounts, callback) {
  ...
  this.data.initialize(function(err) {
    ...
    self.latestBlock(function(err, block) {
      ...
      // Jul: remove this entirely as it is for testing only
      if (options.time) {
...
BlockchainDouble.prototype.createVMFromStateTrie = function(state, activatePrecompiles) {
  ...
  const common = Common.forCustomChain(
    ...
    {
      name: "ganache",
      // Jul: network and chain id should be unique for whole chain
...
BlockchainDouble.prototype.processNextBlock = function(timestamp, callback) {
  ...
  this.createBlock(function(err, block) {
    ...
    // Jul: I think following whole block should be deleted. Seems like it is for overriding block timestamp for testing purposes
    if (timestamp) {
...
// Jul: this method seems to be needed only for testing
